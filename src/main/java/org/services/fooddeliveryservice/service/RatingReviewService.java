...
public class RatingReviewService {
    ...
    public IdResponse rate(Long orderId, RatingReviewRequest request, AppUser customer) {
        FoodOrder order = orderRepository.findById(orderId).orElseThrow(() -> new ResourceNotFoundException("Order not found"));
        if (!order.getCustomer().getId().equals(customer.getId())) {
            throw new UnauthorizedResourceAccessException("Customer cannot rate another customer's order");
        }
        if (order.getStatus() != OrderStatus.DELIVERED) {
            throw new InvalidRequestException("Rating is allowed only after delivery", "RATING_NOT_ALLOWED");
        }
        if (ratingReviewRepository.existsByOrderId(orderId)) {
            throw new ResourceConflictException("Order already rated", "DUPLICATE_RATING");
        }
